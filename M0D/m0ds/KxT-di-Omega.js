...
(()=>{'use strict';
  ...
  // substitui tokens de forma segura: :icon-name: -> <svg class="svg-icon"><use href="#icon-icon-name"></use></svg>
  ...
  const processTabelista = (root=document) => {
    ...
    blocks.forEach(block => {
      ...
      // Reconstrói o HTML do bloco
      block.innerHTML = headHtml + tableHtml;
      // Adiciona botões de tool de volta (opcional, mas o CSS cobre)
      // Nota: processInline rodará depois para transformar [chips] dentro das células
    });
  };
